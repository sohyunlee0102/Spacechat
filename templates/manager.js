...
document.addEventListener('DOMContentLoaded', function() {
    ...
    menuIcon.addEventListener('click', function () {
        ...
        overlay.style.display = chatSidebar.classList.contains('active') ? 'block' : 'none';
        chatContent.classList.toggle('darken');
    });

    overlay.addEventListener('click', function () {
        chatSidebar.classList.remove('active');
        overlay.style.display = 'none';
        chatContent.classList.remove('darken');
    });
});
...
async function syncWithGoogleCalendar() {
    ...
    const url = 'http://spacechat.co.kr:60000/google_login';
    const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
...
//    window.open('http://spacechat.co.kr:60000/google_login', 'Google Login', 'width=600,height=700');
